...
for (maxgap in maxgaps){
  ...
  pqlseq_res <- readRDS("all_pqlseq_res_snp.rds")
  ...
  regions <- na.omit(unique(pqlseq_res[,c("chr_snp", "snp", "chr"), drop = F]))
  ...
  # rownames(region_metaData) <- region_metaData$region
  
  write_rds(x = region_metaData, file = paste0("metaData_SNPs/metaData_SNPs.rds"))
