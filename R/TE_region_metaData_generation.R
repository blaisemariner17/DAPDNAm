...
TE_region_metaData_generation <- function(regions,
  ...
  for (region in rownames(regions)) {
    ...
    if ("LINE" %in% hit$class){line = 1} else {line = 0}
    if ("SINE" %in% hit$class){sine = 1} else {sine = 0}

    if (i == 1){
      region_metaData <- data.frame("region" = region,"LINE" = line,"SINE" = sine,"LINE_id" = paste(unique(hit$LINE_id), collapse = "_"),"SINE_id" = paste(unique(hit$SINE_id), collapse = "_"))
      i = 2
    } else {
      region_metaData <- rbind(region_metaData, data.frame("region" = region,"LINE" = line,"SINE" = sine,"LINE_id" = paste(unique(hit$LINE_id), collapse = "_"),"SINE_id" = paste(unique(hit$SINE_id), collapse = "_")))
    }
  }
