...
#' @param frac_testing testing fraction if you want
...
clock_simulate <- function(alph,
  ...
  if (is.null(sampling)){
    sampling <- sample(unique(metaData$lid_pid), as.integer(frac_testing*length(unique(metaData$lid_pid))))
  }
  ...
  all_res_list <- list()
  all_res_list[["metaData"]] <-list_res[['metaData']]# rbind(list_res[['metaData']], list_res_TE[['metaData']],list_res_PR[['metaData']], list_res_CpGIs[['metaData']],list_res_rand[['metaData']])
  ...
  all_res_list[["clock_results"]] <-list_res[['clock_results']]# rbind(list_res[['clock_results']], list_res_TE[['clock_results']],list_res_PR[['clock_results']], list_res_CpGIs[['clock_results']],list_res_rand[['clock_results']])
