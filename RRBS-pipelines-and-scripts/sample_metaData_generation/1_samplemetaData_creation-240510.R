...
statsQC <- read.csv("/scratch/nsnyderm/dap_rrbs/dap_rrbsQC_stats.txt", sep = " ")

#### Load in the DAP, Shiny, and Clinical metadata
#DAP data
load("/home/bmarine2/colData_metaData_final_240216/DAP_2023_DogOverview_v1.0.RData")
...
#clinical data
clinical_metaData <- read.csv("/home/bmarine2/colData_metaData_final_240216/DAP_2023_SamplesResults_Metadata_v1.0.csv")
...
pauls_precision <- read.csv("/home/bmarine2/colData_metaData_final_240216/precision240301-fromPaul.csv")
...
###########################################################################################################################
###########################################################################################################################
############################## some elab formatting is fucked up so might be needed #######################################
###########################################################################################################################
###########################################################################################################################
# 5/13/24 noah fixed this so we're good for now...
...
#predicted height
predicted_height <- read.csv("/home/bmarine2/colData_metaData_final_240216/precision_dogs_N-837_bodysize.phenotypes-240214.csv")
...
# write.csv(file = "metadata_samples/unique_dog_ids.csv", x = unique(metaData[,c("dog_id")]))
...
metaData <- metaData[!metaData$lid_pid %in% mismatches$lid_pid,]
metaData$Cohort_group[is.na(metaData$Cohort_group)] <- "idk"
...
dim(p1)
readr::write_rds("metadata_samples/P1-DAP-metaData-240510.rds", x = p1)

p2 <- metaData[metaData$Cohort == "precision_2",]
...
dim(p2)
readr::write_rds("metadata_samples/P2-DAP-metaData-240510.rds", x = p2)

p3 <- metaData[metaData$Cohort == "precision_3",]
...
dim(p3)
readr::write_rds("metadata_samples/P3-DAP-metaData-240510.rds", x = p3)
