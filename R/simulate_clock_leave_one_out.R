...
#' @export clock_simulate_leave_one_out

clock_simulate_leave_one_out <- function(alph,
                           metaData,
                           region_metaData,
                           perc_meth,
                           region_metaData_te,
  ...
  for (dog_id in unique(metaData$dog_id)){
    sampling <- dog_id
    ...
    if (dog_id == unique(metaData$dog_id)[1]){
      ...
      all_res_list[["metaData"]]$dog_id <- dog_id
      ...
      all_res_list[["clock_results"]]$dog_id <- dog_id
    } else {
      all_res_list_ <- list()
      all_res_list_[["metaData"]] <- rbind(list_res[['metaData']], list_res_TE[['metaData']],list_res_PR[['metaData']], list_res_CpGIs[['metaData']],list_res_rand[['metaData']])
      all_res_list_[["metaData"]]$alpha <- alph
      all_res_list_[["metaData"]]$dog_id <- dog_id
      # all_res_list_[["region_metaData"]] <- rbind(list_res[['region_metaData']], list_res_TE[['region_metaData']],list_res_PR[['region_metaData']], list_res_CpGIs[['region_metaData']],list_res_rand[['region_metaData']])
      all_res_list_[["clock_results"]] <- rbind(list_res[['clock_results']], list_res_TE[['clock_results']],list_res_PR[['clock_results']], list_res_CpGIs[['clock_results']],list_res_rand[['clock_results']])
      all_res_list_[["clock_results"]]$dog_id <- dog_id

      all_res_list[['metaData']] <- rbind(all_res_list[['metaData']],all_res_list_[['metaData']])
      all_res_list[['clock_results']] <- rbind(all_res_list[['clock_results']],all_res_list_[['clock_results']])
      # all_res_list[['region_metaData']] <- rbind(all_res_list[['region_metaData']],all_res_list_[['region_metaData']])
    }
  }
  return(all_res_list)
}
